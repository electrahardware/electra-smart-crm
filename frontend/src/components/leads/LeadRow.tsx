...
export default function LeadRow({ lead, index, selected, onToggle, onView, onEdit, onDelete }: Props) {
  ...
  const lastEditAge = (() => { if (!lead.lastEditedAt) return ""; const days = Math.floor((new Date(new Date().setHours(0,0,0,0)).getTime() - new Date(new Date(lead.lastEditedAt).setHours(0,0,0,0)).getTime()) / 86400000); if (days <= 0) return "border-emerald-800 bg-emerald-950 text-emerald-300"; if (days >= 2 && days <= 3) return "border-amber-800 bg-amber-950 text-amber-200"; if (days >= 7) return "border-red-800 bg-red-950 text-red-200"; return "border-zinc-700 bg-zinc-800 text-zinc-200"; })();
  ...
  return <tr onClick={onView} onDoubleClick={(event) => { event.stopPropagation(); onEdit(); }} className={`group cursor-pointer border-b border-zinc-100 transition-all duration-200 ${selected ? "bg-red-50/70" : index % 2 ? "bg-zinc-50/65" : "bg-white"} hover:bg-red-50/70`}>
    ...
    <td className="px-4 py-4"><div className="flex min-w-0 items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-zinc-900 text-xs font-bold text-white ring-4 ring-zinc-100">{initials}</span><div className="min-w-0"><p className="truncate font-semibold text-zinc-900 group-hover:text-[#be171d]">{lead.customerName || "Unnamed lead"}</p><p className="mt-1 text-xs text-zinc-500">{formatDate(lead.leadDate || lead.createdAt)}</p></div></div></td>
    <td className="px-4 py-4"><div className="space-y-1">{numbers.length ? numbers.map((mobile) => <button key={mobile} title="Copy mobile number" onClick={(event) => copy(event, mobile)} className="block text-left text-sm font-medium text-zinc-700 hover:text-[#e31e24] hover:underline">{mobile}</button>) : <span className="text-sm text-zinc-400">—</span>}</div></td>
    <td className="px-4 py-4 text-sm text-zinc-600">{lead.city || "—"}</td>
      <td className="px-4 py-4"><span className="inline-flex whitespace-nowrap items-center gap-2 rounded-full border border-white/10 bg-[#252a31] px-2.5 py-1 text-xs font-medium text-zinc-100 shadow-sm"><i className="h-1.5 w-1.5 rounded-full bg-[#e31e24]" />{lead.leadOwner || "Unassigned"}</span></td>
    <td className="px-4 py-4"><StatusBadge status={lead.status} /></td>
    <td className="px-4 py-4">{lead.lastEditedAt ? <span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${lastEditAge}`}>{formatDate(lead.lastEditedAt)}</span> : <span className="text-sm text-zinc-400">—</span>}</td>
