...
function KpiCard({ title, value, icon: Icon, note, alert = false, onClick }: { title: string; value: string | number; icon: InsightIcon; note: string; alert?: boolean; onClick: () => void }) {
  return <button type="button" onClick={onClick} className="premium-stat-card group min-w-0 p-4 text-left transition hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-4 focus:ring-red-500/15 sm:p-5">
...
function Trend({ rows, onSelect }: { rows: { month: string; count: number }[]; onSelect: (month: string) => void }) {
  ...
  return <div><button type="button" onClick={() => data.at(-1) && onSelect(data.at(-1)!.month)} className="h-40 w-full rounded-xl border border-zinc-100 bg-gradient-to-b from-red-50/50 to-white p-3 text-left transition hover:border-red-200 focus:outline-none focus:ring-4 focus:ring-red-500/15" aria-label="Open latest monthly lead trend"><svg className="h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none"><line x1="0" x2="100" y1="90" y2="90" stroke="#e4e4e7" strokeWidth=".7"/><line x1="0" x2="100" y1="55" y2="55" stroke="#f4f4f5" strokeWidth=".7"/><polyline points={points} fill="none" stroke="#e31e24" strokeWidth="2.5" vectorEffect="non-scaling-stroke" strokeLinecap="round" strokeLinejoin="round"/></svg></button><div className="mt-2 flex justify-between gap-1 text-[10px] text-zinc-400">{data.map((item) => <button type="button" className="truncate hover:text-[#e31e24] focus:outline-none focus:underline" onClick={() => onSelect(item.month)} key={item.month}>{item.month}</button>)}</div></div>;
}

function Donut({ items, onSelect }: { items: ChartItem[]; onSelect: (item: ChartItem) => void }) {
  ...
  return <div className="flex items-center gap-5"><div className="relative grid h-32 w-32 shrink-0 place-items-center rounded-full" style={{ background: `conic-gradient(${gradient || "#e4e4e7 0 100%"})` }}><div className="grid h-[5.25rem] w-[5.25rem] place-items-center rounded-full bg-white text-center"><b className="text-2xl tracking-[-.04em]">{total}</b><span className="text-[10px] uppercase tracking-wider text-zinc-500">Leads</span></div></div><div className="min-w-0 space-y-2">{items.slice(0, 4).map((item, index) => <button type="button" onClick={() => onSelect(item)} className="flex w-full items-center justify-between gap-4 rounded px-1 py-0.5 text-xs transition hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-500/30" key={item.name}><span className="flex min-w-0 items-center gap-2 text-zinc-600"><i className="h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: colors[index] }} /><span className="truncate">{item.name}</span></span><b>{item.value}</b></button>)}</div></div>;
}

function monthRange(month: string) {
  const [year, number] = month.split("-").map(Number);
  if (!year || !number) return "";
  const start = new Date(year, number - 1, 1); const end = new Date(year, number, 0);
...
export default function PremiumInsights({ dashboard, analytics }: { dashboard: DashboardData | null; analytics: AnalyticsResponse | null }) {
  ...
  const toLeads = (params = "") => navigate(`/leads${params ? `?${params}` : ""}`);
  const selectMonth = (month: string) => { const params = monthRange(month); toLeads(params); };
  return <section className="mb-10"><div className="mb-5 flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-[#e31e24]">Performance center</p><h2 className="mt-1 text-2xl font-bold tracking-[-.035em]">Sales at a glance</h2></div><p className="hidden text-sm text-zinc-500 sm:block">Live data from your CRM</p></div>
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5">
      <KpiCard title="Total Leads" value={dashboard?.totalLeads ?? 0} icon={Users} note={`${analytics?.newToday ?? 0} added today`} onClick={() => toLeads()} />
      <KpiCard title="Today's Calls" value={analytics?.completedToday ?? 0} icon={PhoneCall} note="Completed follow-ups" onClick={() => toLeads("followup=CompletedToday")} />
      <KpiCard title="Pending Followups" value={pending} icon={Clock3} note="Needs attention" alert onClick={() => toLeads("followup=Pending")} />
      <KpiCard title="Today's Followups" value={analytics?.todayFollowups ?? 0} icon={CalendarCheck2} note="Scheduled today" onClick={() => navigate("/followups")} />
      <KpiCard title="Overdue" value={analytics?.overdue ?? 0} icon={Activity} note="Follow up now" alert onClick={() => navigate("/followups?filter=overdue")} />
      <KpiCard title="Upcoming" value={Math.max((analytics?.weekLeads ?? 0) - (analytics?.newToday ?? 0), 0)} icon={TrendingUp} note="This week's flow" onClick={() => toLeads("followup=Upcoming")} />
      <KpiCard title="Conversion" value={`${conversion}%`} icon={Funnel} note={`${converted} customer leads`} onClick={() => toLeads("status=Customer")} />
      <KpiCard title="Revenue" value={`₹${Number(dashboard?.pipelineValue ?? 0).toLocaleString()}`} icon={CircleDollarSign} note="Pipeline value" onClick={() => toLeads("sort=value")} />
      <KpiCard title="Monthly Growth" value={`${growth >= 0 ? "+" : ""}${growth}%`} icon={TrendingUp} note="vs yesterday" alert={growth < 0} onClick={() => toLeads(`fromDate=${new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().slice(0, 10)}`)} />
    </div>
    <div className="mt-5 grid gap-5 xl:grid-cols-12"><article className="premium-surface p-5 xl:col-span-7"><div className="mb-5 flex items-center justify-between"><div><h3 className="font-bold">Monthly Lead Trend</h3><p className="mt-1 text-sm text-zinc-500">Monthly performance area view</p></div><span className="premium-icon-tile"><TrendingUp size={18}/></span></div><Trend rows={analytics?.monthlyLeads ?? []} onSelect={selectMonth}/></article><article className="premium-surface p-5 xl:col-span-5"><div className="mb-5 flex items-center justify-between"><div><h3 className="font-bold">Lead Status Distribution</h3><p className="mt-1 text-sm text-zinc-500">Current sales pipeline mix</p></div><span className="premium-icon-tile"><PieChart size={18}/></span></div><Donut items={statuses} onSelect={(item) => toLeads(`status=${encodeURIComponent(item.name)}`)}/></article><article className="premium-surface p-5 xl:col-span-5"><h3 className="font-bold">Lead Source</h3><p className="mb-5 mt-1 text-sm text-zinc-500">Opportunity origin</p><Bars items={source} onSelect={(item) => toLeads(`source=${encodeURIComponent(item.name)}`)}/></article><article className="premium-surface p-5 xl:col-span-7"><h3 className="font-bold">Owner Performance</h3><p className="mb-5 text-sm text-zinc-500">Assigned lead volume</p><Bars items={owners} onSelect={(item) => toLeads(`owner=${encodeURIComponent(item.name)}`)}/></article></div>
    <div className="mt-5 grid gap-5 lg:grid-cols-2"><article className="premium-surface p-5"><h3 className="mb-1 font-bold">Sales Funnel</h3><p className="mb-5 text-sm text-zinc-500">Lead stage distribution</p><Bars items={statuses} onSelect={(item) => toLeads(`status=${encodeURIComponent(item.name)}`)}/></article><article className="premium-surface p-5"><h3 className="mb-1 font-bold">Monthly Performance</h3><p className="mb-5 text-sm text-zinc-500">Lead volume over time</p><Trend rows={analytics?.monthlyLeads ?? []} onSelect={selectMonth}/></article></div>
  </section>;
}
