...
export async function parseElectraExcel(
  ...
  const sheetRows = XLSX.utils.sheet_to_json<CellValue[]>(
    worksheet,
    {
      header: 1,
      raw: true,
...
for (let i = 1; i <= 20; i++) {
  ...
  if (dataRows.length > MAX_ROWS) {
    throw new Error(
      "Excel file cannot contain more than 10000 rows."
    );
  }
  ...
  getCellText(
    ...
    secondaryMobile:
  mobileNumbers.secondaryMobile ?? undefined,

    whatsapp:
  whatsappNumbers.primaryMobile ?? undefined,

    shopName: getCellText(
      row,
      columnIndexes,
      "shopName"
    ),
    ...
    leadOwner: getCellText(
      row,
      columnIndexes,
      "leadOwner"
    ),

    leadSource: getCellText(
      row,
      columnIndexes,
      "leadSource"
    ),
